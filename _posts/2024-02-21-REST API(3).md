---
title: REST API(2) Event 생성 API 구현 - Event Controller 테스트 만들기.
...
* **linkTo()** 메서드를 통해 link를 만들어 줄 수 있다. `methodOn(EventController.class).createEvent())` 는 **`EventController`** 클래스 안에 메서드 **`createEvent()`**에 링크를 만든다는 의미이다.<br>`linkTo(methodOn(EventController.class).createEvent()).slash("{id}").toUri();` 👉 `http://localhost/api/events/{id}`  
